...
export const generateImage = async (prompt: string, engine: 'Standard' | 'Anime' = 'Standard'): Promise<string> => {
  try {
    ...
    if (engine === 'Anime') {
      finalPrompt = `Create a standalone anime artwork in manga style. Subject: ${prompt}.
Strict rules:
- Output ONLY the artwork on an artboard; DO NOT depict products or mockups.
- Absolutely NO shirts, hoodies, fabric, apparel, mugs, bottles, phone cases, posters, canvases, packaging, labels, mannequins, people, hands, or a body wearing clothing.
- NO brand tags, hems, seams, collars, necklines, or product wrinkles.
- NO text or watermarks.
Background:
- Provide a cohesive, visually interesting background (soft gradient, subtle abstract shapes, light atmospheric scene) that fills the canvas edge‑to‑edge.
- NEVER use a checkerboard/transparency pattern; do not leave transparent areas.
- Keep background tasteful and not busy so the subject stands out.`;
    } else {
      finalPrompt = `Create a standalone artwork (not a product mockup). Subject: ${prompt}.
Strict rules:
- Output ONLY the artwork on an artboard; DO NOT depict products or mockups.
- Absolutely NO shirts, hoodies, fabric, apparel, mugs, bottles, phone cases, posters, canvases, packaging, labels, mannequins, people, hands, or a body wearing clothing.
- NO brand tags, hems, seams, collars, necklines, or product wrinkles.
- NO text or watermarks.
Background:
- Include a tasteful background with depth (soft gradient / subtle abstract geometry / light bokeh / minimal atmospheric scene) that fills the canvas edge‑to‑edge.
- Do NOT use checkerboard or transparency; avoid plain white/gray blocks; keep it interesting but not cluttered.`;
    }
    ...
    if (response.generatedImages && response.generatedImages.length > 0) {
      ...
      // Return the generated PNG. Prompts above instruct the model to avoid checkerboard/transparent backgrounds.
